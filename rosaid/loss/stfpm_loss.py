# SRC: https://github.com/open-edge-platform/anomalib/blob/main/src/anomalib/models/image/stfpm/loss.py
...
class STFPMLoss(nn.Module):
    ...
    def compute_layer_loss(
        ...
        """
        if len(teacher_feats.shape) != 4 or len(student_feats.shape) != 4:
            norm_teacher_features = F.normalize(teacher_feats)
            norm_student_features = F.normalize(student_feats)
            return (0.5 / teacher_feats.shape[-1]) * self.mse_loss(
